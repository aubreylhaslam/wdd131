...
const oldtemplink = document.querySelector("#oldtemp");

oldtemplink.addEventListener("click", () => {
    createTempleCard(temples.filter(temple => {
        const dedicationDate = new Date(temple.dedicated);
        return dedicationDate.getFullYear() < 1900;
    }))
    document.querySelector("#page").innerHTML = "Old Temples";
})

const newtemplink = document.querySelector("#newtemp");

newtemplink.addEventListener("click", () => {
    createTempleCard(temples.filter(temple => {
        const dedicationDate = new Date(temple.dedicated);
        return dedicationDate.getFullYear() > 2000;
    }))
    document.querySelector("#page").innerHTML = "New Temples";
})

const largetemplink = document.querySelector("#largetemp");

largetemplink.addEventListener("click", () => {
    createTempleCard(temples.filter(temple => temple.area >= 90000)
    )
    document.querySelector("#page").innerHTML = "Large Temples";
});

const smalltemplink = document.querySelector("#smalltemp");

smalltemplink.addEventListener("click", () => {
    createTempleCard(temples.filter(temple => temple.area <= 10000)
    )
    document.querySelector("#page").innerHTML = "Small Temples";
});
